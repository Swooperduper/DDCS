    ...
    if (intCargoType === "unpack") {
        ...
        if (curUnit.inAir) {
            ...
            if (curIntCrateType) {
                ...
                const bases = await ddcsController.baseActionRead({});
                for (const base of bases) {
                    const curCheckAllBase = await ddcsController.isPlayerInProximity(base.centerLoc, 3.4, curUnit.playername);
                ...
                if (curIntCrateBaseOrigin === curBaseName) {
                    ...
                    if (curIntCrateType === "BaseRepair") {
                        if (_.some(playerProx)) {
                            await ddcsController.repairBase(curBaseObj, curUnit);
                        } else {
                            await ddcsController.sendMesgToGroup(
                                curUnit.groupId,
                    ...
                    if (curIntCrateType === "CCBuild") {  // serverName, curUnit, curPlayer, intCargoType
                        await ddcsController.getServer();
                        const delCrates = await ddcsController.staticCrateActionRead({playerOwnerId: curPlayer.ucid});
                        for (const crate of delCrates) {
                            if (crateCount > ddcsController.config.maxCrates - 2) {
                                await ddcsController.staticCrateActionDelete({
                                    _id: crate._id
                                });
                                await ddcsController.destroyUnit(crate._id);
                            }
                            crateCount++;
                        }
                        crateObj = {
                            ...
                            country: ddcsController.defCountrys[curUnit.coalition],
                            side: curUnit.coalition,
                            coalition: curUnit.coalition
                        };
                        await ddcsController.spawnLogiCrate(crateObj, true);
                        await ddcsController.unitActionUpdateByUnitId({unitId: curUnit.unitId, intCargoType: ""});
                        await ddcsController.sendMesgToGroup(
                            curUnit.groupId,
    ...
    if (intCargoType === "loadJTAC" || intCargoType === "loadBaseRepair" || intCargoType === "loadCCBuild") {
        if (curUnit.inAir) {
            ...
            if (_.some(playerProx)) {
                ...
                const aliveLogistics = await ddcsController.unitActionRead({name: curBaseName + " Logistics", dead: false});
                if (aliveLogistics.length > 0 || _.includes(curBaseName, "Carrier")) {
                    if (intCargoType === "loadJTAC") {
                        await ddcsController.unitActionUpdateByUnitId({
...
export async function loadTroops(unitId: string, troopType: string) {
    const units = await ddcsController.unitActionRead({unitId});
    ...
    if (curUnit.inAir) {
        ...
        const playerProx: any [] = [];
        const bases = await ddcsController.baseActionRead({baseType: "MOB", side: curUnit.coalition});
        for (const base of bases) {
            const curCheckAllBase = await ddcsController.isPlayerInProximity(base.centerLoc, 3.4, curUnit.playername);
            playerProx.push(curCheckAllBase);
        }
        if (_.some(playerProx)) {
            await ddcsController.unitActionUpdateByUnitId({unitId, troopType});
            await ddcsController.sendMesgToGroup(
                curUnit.groupId,
            ...
            const secondBases = await ddcsController.baseActionRead({});
            const checkAllSecondBase: any[] = [];
            let curLogistic: any;
            const aliveBases = await ddcsController.unitActionRead({
                _id:  /Logistics/,
                dead: false,
                coalition: curUnit.coalition
            });
            for (const base of secondBases) {
                ...
                if (!!curLogistic) {
                    checkAllSecondBase.push( await ddcsController.isPlayerInProximity(
...
export async function menuCmdProcess(sessionName: string, pObj: any) {
    ...
    const units = await ddcsController.unitActionRead({unitId: pObj.unitId});
    const curUnit = _.get(units, 0);
    if (curUnit) {
        const player = await ddcsController.srvPlayerActionsRead({name: curUnit.playername});
        ...
        if (curPlayer) {
            ...
            switch (pObj.cmd) {
                case "serverTimeLeft":
                    await ddcsController.timeLeft(curUnit);
                    await ddcsController.setSideLockFlags();
                    break;
                case "lookupAircraftCosts":
                    await ddcsController.lookupAircraftCosts(curPlayer.ucid);
                    break;
                case "lookupLifeResource":
                    await ddcsController.lookupLifeResource(curPlayer.ucid);
                    break;
                case "resourcePoints":
                    await ddcsController.checkResourcePoints(curPlayer);
                    break;
                case "unloadExtractTroops":
                    if (curUnit.inAir) {
                        ...
                        if (exports.isTroopOnboard(curUnit)) {
                            ...
                            const bases = await ddcsController.baseActionRead({baseType: "MOB", side: curUnit.coalition});
                            for (const base of bases) {
                                playerProx.push(await ddcsController.isPlayerInProximity(
                                    base.centerLoc,
                                    3.4,
                                    curUnit.playername
                                ));
                            ...
                            if (_.some(playerProx)) {
                                ...
                                const curTroops: any[] = [];
                                const delUnits = await ddcsController.unitActionRead({
                                    playerOwnerId: curPlayer.ucid,
                                    isTroop: true,
                                    dead: false
                                });
                                for (const unit of delUnits) {
                                    ...
                                    await ddcsController.destroyUnit(unit.name);
                                }

                                curSpawnUnit = _.cloneDeep(await ddcsController.getRndFromSpawnCat(
                                    curUnit.troopType,
                                    curUnit.coalition,
                                    false,
                                    true
                                ...
                                for (
                                    let x = 0;
                                    x < curSpawnUnit.config[ddcsController.config.timePeriod].spawnCount;
                                    x++
                                ) {
                                ...
                                ;
                                await ddcsController.spawnLogiGroup(curTroops, curUnit.coalition);
                                await ddcsController.sendMesgToGroup(
                                    curUnit.groupId,
                            ...
                            const troopUnits = await ddcsController.getTroopsInProximity(curUnit.lonLatLoc, 0.2, curUnit.coalition);
                            const curTroop = troopUnits[0];
                            if (curTroop) {
                                // pickup troop
                                const grpUnits = await ddcsController.unitActionRead({
                                    groupName: curTroop.groupName,
                                    isCrate: false,
                                    dead: false
                                });
                                for (const curTroopUnit of grpUnits) {
                                    await ddcsController.destroyUnit(curTroopUnit.name);
                                }
                                await ddcsController.unitActionUpdateByUnitId({
                                    unitId: pObj.unitId,
                ...
                case "isCrateOnboard":
                    ...
                    break;
                case "unpackCrate":
                    const logiProx = await ddcsController.getLogiTowersProximity(curUnit.lonLatLoc, 0.8, curUnit.coalition);
                    if (logiProx.length) {
                        await ddcsController.sendMesgToGroup(
                            curUnit.groupId,
                        ...
                        if (curUnit.inAir) {
                            ...
                            if (curUnit.inAir) {
                                ...
                                const chkPlayer = await ddcsController.srvPlayerActionsRead({name: curUnit.playername});
                                const curChkPlayer = chkPlayer[0];
                                if (curChkPlayer) {

                                    const crateUpdate = await ddcsController.staticCrateActionRead({});
                                    const sendClient = {
                                        action: "CRATEUPDATE",
                ...
                case "Mortar Team":
                    ...
                    break;
                case "acquisitionCnt":
                    const unitsOwned = await ddcsController.unitActionRead({
                        playerOwnerId: curPlayer.ucid,
                        isCrate: false,
                        isTroop: false,
                        dead: false
                    });
                    const serverInfo = await ddcsController.getServer();
...
export async function spawnAtkHeli(curUnit: ddcsController.IUnit, curPlayer: ddcsController.ISrvPlayers, heliType: string, rsCost: number) {
    ...
    const spentPoints = await ddcsController.spendResourcePoints(curPlayer, rsCost, "AtkHeli", heliObj);
    if (spentPoints) {
        await ddcsController.spawnAtkChopper(curUnit, heliObj);
    }
}

export async function spawnAWACS(curUnit: ddcsController.IUnit, curPlayer: ddcsController.ISrvPlayers, awacsType: string, rsCost: number) {
    ...
    const spentPoints = await ddcsController.spendResourcePoints(curPlayer, rsCost, "AWACS", awacsObj);
    if (spentPoints) {
        await ddcsController.spawnAWACSPlane(curUnit, awacsObj);
    }
}
...
export async function spawnBomber(curUnit: any, curPlayer: any, bomberType: string, rsCost: number) {
    ...
    const spentPoints = await ddcsController.spendResourcePoints(curPlayer, rsCost, "Bomber", bomberObj);
    if (spentPoints) {
        await ddcsController.spawnBomberPlane(curUnit, bomberObj);
    }
}
...
export async function spawnCrateFromLogi(
    ...
    if (unit.inAir) {
        ...
        const serverInfo = await ddcsController.getServer();
        const player = await ddcsController.srvPlayerActionsRead({name: unit.playername});
        const curPlayer = player[0];
        let closeLogi = "";
        const normalCrateBases = await ddcsController.baseActionRead({});
        const checkAllBase: any[] = [];
        let curLogistic;
        const aliveBases = await ddcsController.unitActionRead({_id:  /Logistics/, dead: false, coalition: unit.coalition});
        for (const base of normalCrateBases) {
            curLogistic = _.find(aliveBases, {name: base.name + " Logistics"});
            closeLogi = _.get(base, "name");
            if (!!curLogistic) {
                checkAllBase.push(await ddcsController.isPlayerInProximity(
                    curLogistic.lonLatLoc,
                    0.2,
                    unit.playername
                ));
        ...
        if (_.some(checkAllBase)) {
            ...
            const curCrate = _.find(ddcsController.staticDictionary, {_id: crateType});
            if (curCrate) {
            ...
            crateObj = {
                ...
                country: ddcsController.defCountrys[unit.coalition],
                side: unit.coalition,
...
export async function spawnDefHeli(curUnit: any, curPlayer: any, heliType: string, rsCost: number) {
    ...
    const spentPoints = await ddcsController.spendResourcePoints(curPlayer, rsCost, "DefHeli", heliObj);
    if (spentPoints) {
        await ddcsController.spawnDefenseChopper(curUnit, heliObj);
    }
}
...
export async function spawnTanker(curUnit: any, curPlayer: any, tankerType: string, rsCost: number) {
    ...
    remoteLoc = await ddcsController.getLonLatFromDistanceDirection(curUnit.lonLatLoc, curUnit.hdg, tankerObj.spawnDistance);

    const closeMOBs1 = await ddcsController.getMOBsInProximity(
        remoteLoc,
        safeSpawnDistance,
        ddcsController.enemyCountry[curUnit.coalition]
    );
    const closeMOBs2 = await ddcsController.getMOBsInProximity(
        curUnit.lonLatLoc,
        safeSpawnDistance,
        ddcsController.enemyCountry[curUnit.coalition]
    );
    ...
    if (closeMOBs1.length > 0 || closeMOBs2.length > 0) {
        ...
        const spentPoints = await ddcsController.spendResourcePoints(curPlayer, rsCost, "Tanker", tankerObj);
        if (spentPoints) {
            await ddcsController.spawnTankerPlane(curUnit, tankerObj, curUnit.lonLatLoc, remoteLoc);
        }
    }
}

export async function unpackCrate(playerUnit: any, country: string, type: string, special: string, combo: boolean, mobile: boolean) {
    const curTimePeriod = ddcsController.config.timePeriod || "modern";
    if (playerUnit.inAir) {
        await ddcsController.sendMesgToGroup(
            playerUnit.groupId,
            "G: Please Land Before Attempting Logistic Commands!",
        ...
        const player = await ddcsController.srvPlayerActionsRead({name: playerUnit.playername});
        const curPlayer = player[0];
        const delUnits = await ddcsController.unitActionReadStd({
            playerOwnerId: curPlayer.ucid,
            playerCanDrive: mobile,
            isCrate: false,
            dead: false
        ...
        for (const gUnitKey of Object.keys(grpGroups)) {
            if (curUnit <= tRem) {
                for (const unit of grpGroups[gUnitKey]) {
                    await ddcsController.unitActionUpdateByUnitId({unitId: unit.unitId, dead: true});
                    await ddcsController.destroyUnit(unit.name);
                }
                curUnit++;
            }
        ...
        if (combo) {
            const unitDic = await ddcsController.getUnitDictionary(curTimePeriod);
            const addHdg = 30;
            ...
            await ddcsController.spawnLogiGroup(newSpawnArray, playerUnit.coalition);
            return true;
        } else {
            const unitDic = await ddcsController.getUnitDictionary(curTimePeriod);
            const addHdg = 30;
