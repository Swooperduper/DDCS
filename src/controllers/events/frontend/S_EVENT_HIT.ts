        ...
        for (const shootKey of curKeys ) {
            if (shootingUsers[shootKey].startTime + 3000 < new Date().getTime()) {
                ...
                if (shootObj.iucid || shootObj.tucid) {
                    await ddcsController.sendToAll({payload: {action: "S_EVENT_HIT", data: _.cloneDeep(shootObj)}});
                    await ddcsController.simpleStatEventActionsSave(shootObj);
                }
                if (exports.shootingUsers[shootKey].isOwnedUnit) {
                    await ddcsController.srvPlayerActionsUnitAddToRealScore({
                    ...
                    await ddcsController.srvPlayerActionsAddTempScore({
                        _id: shootObj.iucid,
                        groupId: shootObj.groupId,
                        score: shootObj.score
                    });
                ...
                if (shootObj.tUnit.category === "GROUND") {
                    await ddcsController.baseUnitUnderAttack(shootObj.tUnit);
                    ...
                    await ddcsController.baseUnitUnderAttack(shootObj.tUnit);
                    if (ddcsController.config.inGameHitMessages || exports.shootingUsers[shootKey].isOwnedUnit) {
...
export async function processEventHit(sessionName: string, eventObj: any): Promise<void> {
    ...
    let tPlayer: any;
    const nowTime = new Date().getTime();
    const iUnit = await ddcsController.unitActionRead({unitId: iUnitId});
    const tUnit = await ddcsController.unitActionRead({unitId: tUnitId});
    ...
    const ownerIds = await ddcsController.srvPlayerActionsRead({_id: {$in: oId}});
    iCurObj = {
        sessionName,
    ...
    if (iUnit[0].coalition !== tUnit[0].coalition) {
        const curWeapon = _.find(ddcsController.weaponsDictionary, {_id: eventObj.data.arg7.typeName});
        ...
        if (curWeapon) {
            ...
            if (iCurObj.iucid || iCurObj.tucid || isOwnedUnit) {
                if (_.startsWith(curWeapon.name, "weapons.shells")) {
                    ...
                    iCurObj.msg =
                        ddcsController.side[iUnit[0].coalition] + " " + iPName + " has hit " +
                        ddcsController.side[tUnit[0].coalition] + " " + tPName + " " +
                    ...
                    iCurObj.msg = ddcsController.side[iUnit[0].coalition] + " " + iPName + " has hit " +
                        ddcsController.side[tUnit[0].coalition] + " " + tPName + " with " + curWeaponName +
                        " - +" + curWeapon.score;

                    if (iCurObj.iucid || iCurObj.tucid) {
                        await ddcsController.sendToAll({payload: {
                    ...
                    if (iCurObj.tUnit.category === "GROUND") {
                        await ddcsController.baseUnitUnderAttack(iCurObj.tUnit);
                        if (ddcsController.config.inGameHitMessages) {
                        ...
                        if (ddcsController.config.inGameHitMessages || isOwnedUnit) {
            ...
            iCurObj.msg =
                "A: " + ddcsController.side[iUnit[0].coalition] + " " + iPName + " has hit " +
                ddcsController.side[tUnit[0].coalition] + " " + tPName + " " + shotCount + " times with ? - +10";
