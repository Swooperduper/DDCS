			...
			if (initiatorUnits.length > 0) {
				...
				if (isUnitShotHarmDetected) {
					...
					if (targetUnits.length > 0) {
						...
						const localUnits = await ddcsController.getGroundUnitsInProximity(curTarget.lonLatLoc, 0.4, false);
						...
						const friendlySAMs = localUnits.filter((unit) => {
							...
							return unit.coalition === curTarget.coalition && unitTypeDict.harmDetectChance > 0;
						});
						const maxPercentChanceOfGroup = _.max(
							friendlySAMs.map(
								(sam) => {
									const unitTypeDict = engineCache.unitDictionary.find((unitDict: IUnitDictionary) => unitDict._id === sam.type );
