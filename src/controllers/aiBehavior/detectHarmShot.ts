                    ...
                    if (curTargetLonLat.length > 0) {
                        ...
                        if (localUnits.length > 0) {
                            const friendlySAMs = localUnits.filter((unit) => {
                                const unitTypeDict = engineCache.unitDictionary.find(
                                    (unitDict: IUnitDictionary) => unitDict._id === unit.type
                                );
                            ...
                            if (friendlySAMs.length > 0) {
                                ...
                                if (didRadarDetectHarm) {
                                    ...
                                    for (const groupName of groupNameArray) {
                                        ...
                                        // harm detected radars in area around target go dark immediately
                                        // (simulates radars general area fired at)
