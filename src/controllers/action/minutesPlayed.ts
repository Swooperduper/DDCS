...
export async function checkCurrentPlayerBalance(): Promise<ddcsController.IPlayerBalance>  {
    let sideState: ddcsController.IPlayerBalance = {
        underdog: 0,
        ratio: 1
    };
    const latestCampaignArray = await ddcsController.campaignsActionsReadLatest();
    ...
    if (totalCampaignTime > ddcsController.time.oneHour) {
...
export async function updateLatestCampaign(): Promise<void> {
    ...
    if (campaign) {
        const campSessions = await ddcsController.sessionsActionsRead({campaignName: campaign[0].name});
        ...
        await ddcsController.campaignsActionsUpdate({
...
export async function recordFiveMinutesPlayed() {
    ...
    const unitsNewThan = new Date().getTime() - ddcsController.time.fourMins;
    const playerArray = await ddcsController.srvPlayerActionsRead({
...
export async function resetMinutesPlayed() {
    ...
    if (latestSession.length > 0) {
        ...
        for (const player of playerArray) {
            await ddcsController.srvPlayerActionsResetMinutesPlayed({
                _id: player._id,
                side: player.side
            });
        }
