...
export async function baseUnitUnderAttack(unit: ddcsController.IUnit): Promise<void> {
    if (unit.category === "GROUND") {
        const closestBases = await ddcsController.getBasesInProximity(unit.lonLatLoc, 18, unit.coalition);
        if (closestBases) {
