...
export async function resetKickTimer(curPlayer: typings.ISrvPlayers): Promise<void> {
    const curPlayerDB = await localDb.srvPlayerActionsRead({ _id: curPlayer.ucid });
    await localDb.srvPlayerActionsUnsetGicTimeLeft({_id: curPlayerDB[0].ucid });
}
...
export async function processKick(
    ...
    if (newLifeCount !== 0) {
        ...
        if (curPlayerUnit) {
            await playerLib.sendMesgToGroup(curPlayerUnit.groupId, mesg, 60);
        }
        await localDb.srvPlayerActionsUpdate({_id: curPlayer.ucid, gicTimeLeft: newLifeCount});
    } else {
        if (!playerCommObj && isDiscordAllowed) {
