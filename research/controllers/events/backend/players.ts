...
export async function processPlayerEvent(sessionName: string, playerArray: any): Promise<void> {
    ...
    for (const player of playerArray.data) {
        if (player) {
            ...
            const banUser = await localDb.srvPlayerActionsRead({_id: curPlyrUcid, banned: true});
            if (!_.isEmpty(banUser)) {
                console.log("Banning User: ", curPlyrName, curPlyrUcid, player.ipaddr);
                await playerLib.kickPlayer(
                    player.id,
                ...
                const unit = await localDb.unitActionRead({playername: curPlyrName, dead: false});
                ...
                if (curUnit) {
                    ...
                    if (isArtilleryCmdr || isForwardObserver) {
                        const srvPlayer = await localDb.srvPlayerActionsRead({ _id: player.ucid });
                        const curPlayer = srvPlayer[0];
                        if (curPlayer.gciAllowed || isForwardObserver) {
                            if (curPlayer.sideLock === 0) {
                                await localDb.srvPlayerActionsUpdate({
                                    _id: player.ucid,
                            ...
                            if (constants.config.isJtacLocked) {
                                await playerLib.forcePlayerSpectator(player.id, "You are not allowed to use " +
    ...
    for (const data of playerArray.data) {
        ...
        if (curData.ucid) {
            await localDb.srvPlayerActionsUpdateFromServer({
                ...curData,
                _id: curData.ucid,
                playerId: curData.id,
                sessionName
