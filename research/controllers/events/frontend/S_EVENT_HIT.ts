        ...
        for (const shootKey of curKeys ) {
            if (shootingUsers[shootKey].startTime + 3000 < new Date().getTime()) {
                ...
                if (shootObj.iucid || shootObj.tucid) {
                    await webPush.sendToAll({payload: {action: "S_EVENT_HIT", data: _.cloneDeep(shootObj)}});
                    await localDb.simpleStatEventActionsSave(shootObj);
                }
                if (exports.shootingUsers[shootKey].isOwnedUnit) {
                    await localDb.srvPlayerActionsUnitAddToRealScore({
                    ...
                    await localDb.srvPlayerActionsAddTempScore({
                        _id: shootObj.iucid,
                        groupId: shootObj.groupId,
                        score: shootObj.score
                    });
                ...
                if (shootObj.tUnit.category === "GROUND") {
                    await action.baseUnitUnderAttack(shootObj.tUnit);
                    ...
                    await action.baseUnitUnderAttack(shootObj.tUnit);
                    if (constants.config.inGameHitMessages || exports.shootingUsers[shootKey].isOwnedUnit) {
...
export async function processEventHit(sessionName: string, eventObj: any): Promise<void> {
    ...
    let tPlayer: any;
    const nowTime = new Date().getTime();
    const iUnit = await localDb.unitActionRead({unitId: iUnitId});
    const tUnit = await localDb.unitActionRead({unitId: tUnitId});
    ...
    const ownerIds = await localDb.srvPlayerActionsRead({_id: {$in: oId}});
    iCurObj = {
        sessionName,
        ...
        if (curWeapon) {
            ...
            if (iCurObj.iucid || iCurObj.tucid || isOwnedUnit) {
                if (_.startsWith(curWeapon.name, "weapons.shells")) {
                    ...
                    iCurObj.msg =
                        constants.side[iUnit[0].coalition] + " " + iPName + " has hit " +
                        constants.side[tUnit[0].coalition] + " " + tPName + " " +
                    ...
                    iCurObj.msg = constants.side[iUnit[0].coalition] + " " + iPName + " has hit " +
                        constants.side[tUnit[0].coalition] + " " + tPName + " with " + curWeaponName +
                        " - +" + curWeapon.score;

                    if (iCurObj.iucid || iCurObj.tucid) {
                        await webPush.sendToAll({payload: {
                    ...
                    if (iCurObj.tUnit.category === "GROUND") {
                        await action.baseUnitUnderAttack(iCurObj.tUnit);
                        if (constants.config.inGameHitMessages) {
            ...
            iCurObj.msg =
                "A: " + constants.side[iUnit[0].coalition] + " " + iPName + " has hit " +
                constants.side[tUnit[0].coalition] + " " + tPName + " " + shotCount + " times with ? - +10";
