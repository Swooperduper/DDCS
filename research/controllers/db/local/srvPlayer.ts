...
const srvPlayerTable = constants.localConnection.model(process.env.SERVER_NAME + "_srvPlayer", schemas.srvPlayerSchema);

export async function srvPlayerActionsRead(obj: any): Promise<typings.ISrvPlayers[]> {
    return new Promise((resolve, reject) => {
        srvPlayerTable.find(obj, (err, srvPlayer: typings.ISrvPlayers[]) => {
            if (err) { reject(err); }
            resolve(srvPlayer);
        });
    });
...
export async function srvPlayerActionsUpdateFromServer(obj: {
    ...
    return new Promise((resolve, reject) => {
        srvPlayerTable.find({_id: obj._id}, (err, serverObj: typings.ISrvPlayers[]) => {
            if (err) { reject(err); }
...
export async function srvPlayerActionsAddLifePoints(obj: {
    ...
    return new Promise((resolve, reject) => {
        srvPlayerTable.find({_id: obj._id}, (err: any, serverObj: typings.ISrvPlayers[]) => {
            if (err) { reject(err); }
            ...
            const maxLimitedPoints: number = (curTotalPoints > constants.maxLifePoints) ?
                constants.maxLifePoints : curTotalPoints;
            let msg;
            if (serverObj.length > 0) {
                const setObj = {
                ...
                srvPlayerTable.findOneAndUpdate(
                    ...
                    (updateErr, srvPlayer) => {
                        ...
                        if (obj.groupId) {
                            playerLib.sendMesgToGroup( obj.groupId, msg, 5);
                        }
                        resolve();
                    }
                );
    ...
    return new Promise((resolve, reject) => {
        srvPlayerTable.find({_id: obj._id}, (err: any, serverObj: typings.ISrvPlayers[]) => {
            ...
            if (serverObj.length > 0 && serverObj[0].playerId) {
                if (curTotalPoints < 0) {
                    playerLib.forcePlayerSpectator(
                    ...
                    srvPlayerTable.findOneAndUpdate(
                        ...
                        (updateErr, srvPlayer) => {
                            if (updateErr) { reject(updateErr); }
                            playerLib.sendMesgToGroup( obj.groupId, "You Have Just Used " +
            ...
            if (serverObj.length !== 0) {
                ...
                srvPlayerTable.updateOne(
                    ...
                    (updateErr) => {
                        ...
                        playerLib.sendMesgToGroup(obj.groupId, mesg, 15);
                        resolve();
                    }
                );
            } else {
