...
const userAccountTable = remoteConnection.model("useraccounts", schemas.userAccountSchema);
...
export async function userAccountActionsRead(obj: any): Promise<typings.IUserAccount[]> {
    return new Promise((resolve, reject) => {
        userAccountTable.find(obj, (err, useraccount: typings.IUserAccount[]) => {
            if (err) { reject(err); }
            resolve(useraccount);
        });
    });
...
export async function userAccountActionsGetPerm(obj: any): Promise<typings.IUserAccount[]> {
    return new Promise((resolve, reject) => {
        userAccountTable.find({authId: obj}, (err, useraccount: typings.IUserAccount[]) => {
            if (err) { reject(err); }
            resolve(useraccount);
        });
    });
...
export async function userAccountActionsUpdate(obj: any): Promise<void> {
    return new Promise((resolve, reject) => {
        userAccountTable.find({ucid: obj.ucid}, (err, ucidUser: typings.IUserAccount[]) => {
            if (err) {
                reject(err);
            }
            ...
            if (ucidUser.length > 0) {
                ...
                userAccountTable.find({lastIp: obj.lastIp}, (findErr, ipUser: typings.IUserAccount[]) => {
                    if (findErr) {
                        reject(findErr);
                    }
...
export async function userAccountActionsUpdateSocket(obj: any): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
        userAccountTable.find({authId: obj.authId}, (err, authIdUser: typings.IUserAccount[]) => {
            if (err) {
                reject(err);
            }
