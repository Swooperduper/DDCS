...
export async function processTimer(serverSecs: number): Promise<void> {
    maxTime = constants.config.restartTimer;
    mesg = null;
    curSecs = serverSecs;

    if (maxTime > 0) {
        if (serverSecs > (maxTime - (constants.time.oneHour * 4)) && !exports.timerObj.fourHours) {
            mesg = "Server is restarting in 4 hours!";
            exports.timerObj.fourHours = true;
        }
        // 3 hours
        if (serverSecs > (maxTime - (constants.time.oneHour * 3)) && !exports.timerObj.threeHours) {
            mesg = "Server is restarting in 3 hours!";
            exports.timerObj.threeHours = true;
        }
        // 2 hours
        if (serverSecs > (maxTime - (constants.time.oneHour * 2)) && !exports.timerObj.twoHours) {
            mesg = "Server is restarting in 2 hours!";
            exports.timerObj.twoHours = true;
        }
        // 1 hour
        if (serverSecs > (maxTime - constants.time.oneHour) && !exports.timerObj.oneHour) {
        ...
        if (serverSecs > (maxTime - 120) && !exports.timerObj.twoMinutes) {
            ...
            await playerLib.setIsOpenSlotFlag(0);
        }
        // 1 min
        if (serverSecs > (maxTime - 60) && !exports.timerObj.oneMinute) {
            ...
            if (latestSession[0].name) {
                const playerArray = await localDb.srvPlayerActionsRead({ sessionName: latestSession[0].name });
                for (const player of playerArray) {
                    await playerLib.kickPlayer(Number(player.playerId), "Server is now restarting!");
                }
            }
        }
        // restart server
        if (serverSecs > maxTime) {
            ...
            if (mesg) {
                ...
                await playerLib.sendMesgToAll(mesg, 20);
            }
        }
    }
}
