...
export function getWeaponCost(typeName: string, count: number): number {
    ...
    const curWeaponLookup = _.find(constants.weaponsDictionary, {_id: typeName} );
    if (curWeaponLookup) {
...
export async function getPlayerBalance(): Promise<typings.ISrvPlayerBalance> {
    ...
    if (latestSession[0].name) {
        ...
        for (const ePlayer of playerArray) {
            if ((new Date(ePlayer.updatedAt).getTime() + constants.time.oneMin > new Date().getTime()) && ePlayer.slot !== "") {
        ...
        for (const cPlayer of playerBalance.players) {
            if (cPlayer) {
                if (!_.isEmpty(cPlayer.name)) {
                    const cUnit = await localDb.unitActionRead({dead: false, playername: cPlayer.name});
...
export async function lookupLifeResource(playerUcid: string): Promise<void> {
    const srvPlayer = await localDb.srvPlayerActionsRead({_id: playerUcid});
    const curPlayer = _.get(srvPlayer, [0]);
    if (curPlayer) {
        if (curPlayer.name) {
            const cUnit = await localDb.unitActionRead({playername: curPlayer.name});
            const curUnit = cUnit[0];
...
export async function addLifePoints(curPlayer: any, curUnit: any, execAction?: string, addLP?: number): Promise<void> {
    await localDb.srvPlayerActionsAddLifePoints({
        _id: curPlayer._id,
...
export async function removeLifePoints(
    ...
    await localDb.srvPlayerActionsRemoveLifePoints({
        _id: curPlayer._id,
        groupId: curUnit.groupId,
