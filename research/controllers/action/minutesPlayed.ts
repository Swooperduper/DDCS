...
export async function checkCurrentPlayerBalance(): Promise<typings.IPlayerBalance>  {
    ...
    const latestCampaignArray = await localDb.campaignsActionsReadLatest();
    ...
    if (totalCampaignTime > constants.time.oneHour) {
...
export async function updateLatestCampaign(): Promise<void> {
    const campaign = await localDb.campaignsActionsReadLatest();
    if (campaign) {
        const campSessions = await localDb.sessionsActionsRead({campaignName: campaign[0].name});
        ...
        await localDb.campaignsActionsUpdate({
...
export async function updateSession(sessionName: string) {
    ...
    await localDb.sessionsActionsUpdate({
        name: sessionName,
...
export async function recordFiveMinutesPlayed() {
    ...
    const unitsNewThan = new Date().getTime() - constants.time.fourMins;
    const playerArray = await localDb.srvPlayerActionsRead({
    ...
    _.forEach(playerArray, (player) => {
        ...
        processPromise.push(localDb.srvPlayerActionsAddMinutesPlayed({
            _id: player._id,
            minutesPlayed: 5,
            side: player.side
        }));
...
export async function resetMinutesPlayed() {
    ...
    if (latestSession.length > 0) {
        ...
        for (const player of playerArray) {
            await localDb.srvPlayerActionsResetMinutesPlayed({
                _id: player._id,
                side: player.side
            });
        }
