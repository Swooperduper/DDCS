...
export async function spendResourcePoints(
    ...
    if (isNaN(Number(player.slot))) {
        ...
        const cUnit = await localDb.unitActionRead({unitId: Number(player.slot)});
        let mesg;
        let currentObjUpdate: any;
        curUnit = cUnit[0];
        if (curUnit.inAir) {
            const unitExist = await localDb.unitActionRead({_id: "AI|" + itemObj.name + "|"});
            if (unitExist.length > 0 && rsItem === "Tanker") {
                ...
                if (player.side === 1) {
                    if (player.redRSPoints >= rsCost) {
                        ...
                        await localDb.srvPlayerActionsUpdate(currentObjUpdate);
...
export async function checkResourcePoints(player: typings.ISrvPlayers): Promise<void> {
    if (player.name) {
        const cUnit = await localDb.unitActionRead({dead: false, playername: player.name});
        let mesg;
        if (cUnit.length > 0) {
            if (player.side === 1) {
