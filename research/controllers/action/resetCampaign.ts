...
export async function clearCampaignTables(): Promise<void> {
    ...
    await localDb.cmdQueActionsRemoveAll();
    await localDb.staticCrateActionRemoveall();
    await localDb.unitActionRemoveall();
}

export async function restartCampaign(): Promise<void> {
    console.log("ALL TABLES CLEARED OFF, restart");
    if (constants.config.fullServerRestartOnCampaignWin) {
        await shutdown((output: any) => {
            console.log(output);
        });
    } else {
        await action.restartServer();
    }
}
