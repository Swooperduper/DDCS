...
export async function processUnitUpdates(sessionName: string, unitObj: any): Promise<void> {
    if (!exports.lockUpdates) {
        const unit = await localDb.unitActionRead({_id: unitObj.data.name});
        let stParse;
        let iCurObj: any;
        ...
        if (!_.includes(curData.name, "New Static Object")) {
            ...
            if (_.includes(curData.name, "Carrier")) {
                await localDb.baseActionUpdate({_id: curUnitName, centerLoc: curData.lonLatLoc});
            }
            ...
            if ((!_.isEmpty(curUnit) && unitObj.action !== "D")) {
                ...
                await localDb.unitActionUpdate(iCurObj.data);
                ...
                if (curData.category === "STRUCTURE") {
                    await action.setUnitMark(curData);
                }
                ...
                if (curData.name) {
                    ...
                    await localDb.unitActionSave(iCurObj.data);
                    await webPush.sendToCoalition({payload: {
                            action: "C",
                            data: {
                                _id: iCurObj.data._id,
                ...
                if (curData.name) {
                    ...
                    if (iCurObj.data.coalition) {
                        ...
                        webPush.sendToCoalition({payload: _.cloneDeep(iCurObj)});
                    }
                } else {
