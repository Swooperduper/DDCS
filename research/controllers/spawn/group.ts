...
export function getRndFromSpawnCat(
    ...
    const curTimePeriod = constants.config.timePeriod;
    const curEnabledCountrys = constants.side[side] + "Countrys";
    let findUnits;
    const cPUnits: any[] = [];
    let randomIndex;
    ...
    if (!_.isEmpty(useUnitType)) {
        const curComboUnit = _.find(constants.unitDictionary, {type: useUnitType});
        if (curComboUnit && curComboUnit.comboName) {
            findUnits = _.filter(constants.unitDictionary, {comboName: curComboUnit.comboName});
        }
    } else if (curTimePeriod === "modern" && spawnCat === "samRadar") {
        findUnits = _.filter(constants.unitDictionary, {spawnCat: "samRadar", spawnCatSec: "modern", enabled: true});
    } else {
        findUnits = _.filter(constants.unitDictionary, {spawnCat, enabled: true});
    }
...
export function spawnSupportVehiclesOnFarp( baseName: string, side: number ): typing.IUnit[] | void {
    const curBase = _.find(constants.bases, {name: baseName});
    ...
    if (curBase) {
        ...
        for ( const val of sptArray) {
            ...
            const sptUnit = {
                name: baseName + "_" + val,
                lonLatLoc: proxZone.getLonLatFromDistanceDirection(curBase.centerLoc, curAng, 0.05),
                ...curObj
            };
            curAng += 15;
...
export async function spawnSupportBaseGrp( baseName: string, side: number ): Promise<void> {
    let spawnArray: any[] = [];
    const farpBases = _.filter(constants.bases, (baseObj) => {
...
export async function spawnBaseReinforcementGroup(side: number, baseName: string, forceSpawn?: boolean, init?: boolean): Promise<number> {
    ...
    let curCat;
    let curRndSpawn;
    const curServer = constants.config;
    let curSpokeDeg;
    let curSpokeNum;
    let infoSpwn;
    ...
    for (const spawnCats of Object.keys(curBaseSpawnCats)) {
        ...
        for (let i = 0; i < spawnTicks; i++) {
            ...
            if (infoSpwn.spoke) {
                randLatLonInBase = proxZone.getRandomLatLonFromBase(baseName, polyCheck);
                groupedUnits = [];
                ...
                for (let j = _.cloneDeep(centerRadar); j < infoSpwn.secRadarNum + centerRadar; j++) {
                    ...
                    curCat.lonLatLoc = proxZone.getLonLatFromDistanceDirection(randLatLonInBase, curAngle, curCat.spokeDistance / 2);
                ...
                for (let k = infoSpwn.secRadarNum + centerRadar; k < curSpokeNum + centerRadar; k++) {
                    ...
                    curCat.lonLatLoc = proxZone.getLonLatFromDistanceDirection(randLatLonInBase, curAngle, curCat.spokeDistance);
                    curAngle += curSpokeDeg;
                    groupedUnits.push(curCat);
                }
...
export async function spawnSAMNet(side: number, baseName: string, init?: boolean): Promise<void> {
    ...
    const samUnits = await localDb.unitActionRead({$and: [{name: new RegExp(baseName)}, {name: /SAM/}], dead: false});
...
export async function spawnStarSam(
    ...
    randLatLonInBase = (lastLonLat) ? lastLonLat : proxZone.getRandomLatLonFromBase(baseName, "layer2Poly", openStarSAM);
    groupedUnits = [];
    ...
    for (let j = _.cloneDeep(centerRadar); j < _.get(infoSpwn, "secRadarNum") + centerRadar; j++) {
        curCat = {
            ...
            name: "|" + baseName + "|" + openStarSAM + "SAM|" + _.random(1000000, 9999999),
            lonLatLoc: proxZone.getLonLatFromDistanceDirection(randLatLonInBase, curAngle, curCat.spokeDistance / 2)
        };
        curAngle += curSpokeDeg;
        groupedUnits.push(curCat);
    }
    ...
    for (let k = _.get(infoSpwn, "secRadarNum") + centerRadar; k < curSpokeNum + centerRadar; k++) {
        curCat = {
            ...
            lonLatLoc: proxZone.getLonLatFromDistanceDirection(randLatLonInBase, curAngle, curCat.spokeDistance)
        };
        curAngle += curSpokeDeg;
        groupedUnits.push(curCat);
    }
    ...
    curCat = {
        ...
        name: "|" + baseName + "|" + openStarSAM + "SAM|" + _.random(1000000, 9999999),
        lonLatLoc: proxZone.getLonLatFromDistanceDirection(randLatLonInBase, 180, curCat.spokeDistance / 2)
    };
    groupedUnits.push(curCat);
    compactUnits = _.compact(groupedUnits);
    await spawnGroup(compactUnits, baseName, side);
...
export async function spawnLayer2Reinforcements(
    ...
    for (let i = 0; i < curTickCnt; i++) {
        ...
        randLatLonInBase = _.cloneDeep(proxZone.getRandomLatLonFromBase(baseName, "layer2Poly"));
        curCat = {
        ...
        for (let j = 0; j < curSpokeNum; j++) {
            ...
            curUnit = {
                ..._.cloneDeep(curRndSpawn[j]),
                lonLatLoc: proxZone.getLonLatFromDistanceDirection(randLatLonInBase, curAngle, 0.05)
            };
            curAngle += curSpokeDeg;
...
export async function spawnConvoy(
    ...
    for (const units of aIConfig.makeup) {
        curUnit = {
            ...
            country: constants.defCountrys[convoySide],
            speed: "55",
            hidden: false,
            playerCanDrive: false
        };
    ...
    await action.setMissionTask(groupName, JSON.stringify(exports.convoyRouteTemplate(curGrpObj)));
...
export async function spawnCAPDefense(
    ...
    for (const aiMakeup of aIConfig.makeup) {
        ...
        for (let y = 0; y < aiMakeup.count; y++) {
            ...
            if (curCapTemp.type === "AH-1W") {
                curUnit.routeLocs = proxZone.getLonLatFromDistanceDirection(curCapTemp.lonLat, curAngle, 0.15);
                curAngle += 180;
    ...
    const curCMD = spawnGrp(curGroupSpawn, constants.defCountrys[convoySide], curUnit.category);
    const sendClient = {action: "CMD", cmd: [curCMD], reqID: 0};
    const actionObj = {actionObj: sendClient, queName: "clientArray"};
    await localDb.cmdQueActionsSave(actionObj);
    await playerLib.sendMesgToCoalition(
...
export async function spawnDefenseChopper(playerUnitObj: typing.IUnit, unitObj: typing.IUnit): Promise<void> {
    ...
    const friendlyBase = await localDb.baseActionGetClosestFriendlyBase({
        unitLonLatLoc: playerUnitObj.lonLatLoc,
        playerSide: playerUnitObj.coalition
    });
    const patrolDistance = 2;
    friendlyLoc = proxZone.getLonLatFromDistanceDirection(friendlyBase.centerLoc, 0, patrolDistance);
    curGrpObj = {
        ...
        routeLocs: [
            friendlyLoc,
            proxZone.getLonLatFromDistanceDirection(friendlyBase.centerLoc, 45, patrolDistance),
            proxZone.getLonLatFromDistanceDirection(friendlyBase.centerLoc, 90, patrolDistance),
            proxZone.getLonLatFromDistanceDirection(friendlyBase.centerLoc, 135, patrolDistance),
            proxZone.getLonLatFromDistanceDirection(friendlyBase.centerLoc, 180, patrolDistance),
            proxZone.getLonLatFromDistanceDirection(friendlyBase.centerLoc, 225, patrolDistance),
            proxZone.getLonLatFromDistanceDirection(friendlyBase.centerLoc, 270, patrolDistance),
            proxZone.getLonLatFromDistanceDirection(friendlyBase.centerLoc, 315, patrolDistance)
        ]
    };
...
export async function spawnAtkChopper(playerUnitObj: typing.IUnit, unitObj: typing.IUnit): Promise<void> {
    ...
    const enemyBase = await localDb.baseActionGetClosestEnemyBase({
        unitLonLatLoc: playerUnitObj.lonLatLoc,
        playerSide: playerUnitObj.coalition
    });
    const friendlyBase = await localDb.baseActionGetClosestFriendlyBase({
    ...
    friendlyLoc = proxZone.getLonLatFromDistanceDirection(
        friendlyBase.centerLoc,
        proxZone.findBearing(
...
export async function spawnBomberPlane(playerUnitObj: typing.IUnit, bomberObj: any): Promise<void> {
    ...
    const closeBase = await localDb.baseActionGetClosestEnemyBase({
        unitLonLatLoc: playerUnitObj.lonLatLoc,
        playerSide: playerUnitObj.coalition
    });
    remoteLoc = proxZone.getLonLatFromDistanceDirection(closeBase.centerLoc, randomDir, curSpwnUnit.spawnDistance);
    closeLoc = proxZone.getLonLatFromDistanceDirection(closeBase.centerLoc, randomDir, 7);
...
export async function spawnAWACSPlane(playerUnitObj: typing.IUnit, awacsObj: any): Promise<void> {
    ...
    const closeBase = await localDb.baseActionGetClosestBase({ unitLonLatLoc: playerUnitObj.lonLatLoc});
    remoteLoc = proxZone.getLonLatFromDistanceDirection(
...
export async function spawnTankerPlane(
    ...
    const closeBase = await localDb.baseActionGetClosestBase({ unitLonLatLoc: playerLoc});
    curGrpObj = {
        ..._.cloneDeep(curSpwnUnit),
        groupName: curTkrName + "#" + _.random(1000000, 9999999),
        country: curCountry,
...
export async function spawnSupportPlane(baseObj: typing.IBase, side: number): Promise<void> {
    ...
    curSide = constants.defCountrys[side];
    ...
    if (_.includes(baseObj._id, "_MOB") || _.includes(baseObj._id, "_FOB")) {
        ...
        remoteLoc = proxZone.getLonLatFromDistanceDirection(baseLoc, randomDir, 40);
    } else {
        curSpwnUnit = _.cloneDeep(getRndFromSpawnCat("transportAircraft", side, true, true )[0]);
        remoteLoc = proxZone.getLonLatFromDistanceDirection(baseLoc, randomDir, 70);
    }
    curGrpObj = {
        ...curSpwnUnit,
        groupId: grpNum,
...
export async function spawnLogiGroup(spawnArray: typing.IUnit[], side: number): Promise<void> {
    ...
    if (curGrpObj) {
        ...
        if (side === 2 && _.includes(curGrpObj.country, "UKRAINE")) {
            ...
            curSide = (side) ? constants.defCountrys[side] : constants.defCountrys[curGrpObj.coalition];
        }
        ...
        for (const curUnit of sArray) {
            ...
            curSpwnUnit.lonLatLoc = proxZone.getLonLatFromDistanceDirection(curSpwnUnit.lonLatLoc, curAng, 0.05);
            curAng += 15;
...
export async function spawnGroup(spawnArray: any[], baseName?: string, side?: number): Promise<void> {
    ...
    if (curGrpObj) {
        ...
        if (side === 2 && _.includes(curGrpObj.country, "UKRAINE")) {
            ...
            curSide = (side) ? constants.defCountrys[side] :
                curGrpObj.country = constants.defCountrys[curGrpObj.coalition];
            curGrpObj.country = curSide;
        }
        curGroupSpawn = grndUnitGroup( curGrpObj );
        unitNum = _.cloneDeep(grpNum);
        for (const curUnit of sArray) {
            ...
            if (_.isUndefined(curSpwnUnit.lonLatLoc)) {
                curSpwnUnit.lonLatLoc = proxZone.getRandomLatLonFromBase(curBaseName, "unitPoly");
            }
...
export async function spawnNewMapGrps(): Promise<number> {
    ...
    const curServer = constants.config;
    let totalUnitNum;
    const bases = await localDb.baseActionRead({name: {$not: /#/}, enabled: true});
    for (const base of bases) {
        if (!_.includes(base.name, "Carrier")) {
            ...
            totalUnitNum = 0;
            await spawn.spawnLogisticCmdCenter({}, false, base, baseStartSide);
            ...
            if (_.get(base, "baseType") === "MOB") {
                ...
                await spawnRadioTower(
                    {},
                    true,
                    _.find(constants.bases, { name: baseName } ),
                    baseStartSide
                );
            }
...
export async function spawnLogisticCmdCenter(staticObj: any, init: boolean, baseObj?: any, side?: number): Promise<void> {
    ...
    if (_.isUndefined(curGrpObj.lonLatLoc)) {
        curGrpObj.lonLatLoc = proxZone.getRandomLatLonFromBase(baseObj.name, "buildingPoly");
    }
    ...
    const actionObj = {actionObj: sendClient, queName: "clientArray"};
    await localDb.cmdQueActionsSave(actionObj);
    await localDb.unitActionUpdateByName({
        name: curGrpObj.name,
...
export async function spawnRadioTower(staticObj: any, init: boolean, baseObj?: typing.IBase, side?: number): Promise<void> {
    ...
    curGrpObj.country = constants.defCountrys[curGrpObj.coalition];
    if (_.isUndefined(curGrpObj.lonLatLoc)) {
        curGrpObj.lonLatLoc = proxZone.getRandomLatLonFromBase(curBaseName, "buildingPoly");
    }
...
export async function spawnBaseEWR(serverName: string, type: string, baseName: string, side: number): Promise<void> {
    let unitStart: any = {};
    let pCountry = constants.defCountrys[side];
    const curTimePeriod = constants.config.timePeriod;
    const findUnit = _.find(constants.unitDictionary, {_id: type});
    ...
    if (findUnit) {
        ...
        for (let x = 0; x < spawnUnitCount; x++) {
            unitStart = {
                ...
                lonLatLoc: proxZone.getRandomLatLonFromBase(serverName, baseName, "buildingPoly"),
                heading: 0,
                country: pCountry,
                playerCanDrive: false
            };
...
export async function healBase( baseName: string, curPlayerUnit: any): Promise<boolean> {
    const baseUnit = await localDb.baseActionRead({name: baseName});
    if (baseUnit.length > 0) {
        ...
        if (curBase.baseType !== "MOB") {
            ...
            const logiUnit = await localDb.unitActionRead({name: curBase.name + " Logistics", dead: false});
            ...
            const commUnit = await localDb.unitActionRead({name: curBase.name + " Communications", dead: false});
