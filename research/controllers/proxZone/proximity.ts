...
export async function checkUnitsToBaseForCapture(): Promise<void> {
    ...
    for (const base of bases) {
        const sideLabel: any = constants.side[base.side];
        ...
        if (base.side === 1 && _.get(sideArray, [2], []).length > 0) {
            ...
            if (_.get(sideArray, [1], []).length === 0) {
                ...
                await spawn.spawnSupportBaseGrp(base.name, 2);
                await localDb.baseActionUpdateSide({name: base.name, side: 2});
                await action.setbaseSides();
                const aliveLogistics = await localDb.unitActionRead({name: base.name + " Logistics", dead: false});
                if (aliveLogistics.length > 0) {
                    await spawn.spawnLogisticCmdCenter({}, false, base, 2);
                }
                const aliveComms = await localDb.unitActionRead({name: base.name + " Communications", dead: false});
                if (aliveComms.length > 0) {
                    await spawn.spawnRadioTower({}, false, base, 2);
                }
            }
        }
        if (base.side === 2 && _.get(sideArray, [1], []).length > 0) {
            ...
            if (_.get(sideArray, [2], []).length === 0) {
                ...
                await spawn.spawnSupportBaseGrp(base.name, 1);
                await localDb.baseActionUpdateSide({name: base.name, side: 1});
                await action.setbaseSides();
                const aliveLogistics = await localDb.unitActionRead({name: base.name + " Logistics", dead: false});
                if (aliveLogistics.length > 0) {
                    await spawn.spawnLogisticCmdCenter({}, false, base, 1);
                }
            }
        }
        if (base.side === 0 && (_.get(sideArray, [1], []).length > 0 || _.get(sideArray, [2], []).length > 0)) {
            ...
            await spawn.spawnSupportBaseGrp(base.name, unitSide);
            await localDb.baseActionUpdateSide({name: base.name, side: unitSide});
            await action.setbaseSides();
            const aliveLogistics = await localDb.unitActionRead({name: base.name + " Logistics", dead: false});
            if (aliveLogistics.length > 0) {
                await spawn.spawnLogisticCmdCenter({}, false, base, unitSide);
            }
            const aliveComms = await localDb.unitActionRead({name: base.name + " Communications", dead: false});
            if (aliveComms.length > 0) {
                await spawn.spawnRadioTower({}, false, base, unitSide);
            }
        }
    }
    if (!_.isEmpty(bases)) {
        if (campaignState.red === 0 && !serverToDbSync.lockUpdates) {
            console.log("BLUE WON BLUE WON BLUE WON BLUE WON BLUE WON BLUE WON BLUE WON BLUE WON ");
            await remoteDb.serverActionsUpdate({resetFullCampaign: true});
            await serverToDbSync.setLockUpdates(true);
            await action.setTimeToRestart(new Date().getTime() + constants.time.fiveMins);
            await playerLib.sendMesgToAll(
                "Blue has won the campaign, Map will reset in 5 minutes.",
                constants.time.fiveMins
            );
        }
        if (campaignState.blue === 0 && !serverToDbSync.lockUpdates) {
...
export async function getCoalitionGroundUnitsInProximity(
    ...
    return await localDb.unitActionRead({
            dead: false,
            lonLatLoc: {
                $geoWithin: {
                    $centerSphere: [
...
export async function getMOBsInProximity(lonLat: number[], kmDistance: number, side: number): Promise<typing.IBase[]> {
    return await localDb.baseActionRead({
            centerLoc: {
                $near: {
                    $geometry: {
                        type: "Point",
...
export async function getGroundUnitsInProximity(lonLat: number[], kmDistance: number, isTroop: boolean): Promise<typing.IUnit[]> {
    return await localDb.unitActionReadStd({
        dead: false,
        lonLatLoc: {
            $near: {
                $geometry: {
...
export async function getStaticCratesInProximity(
    ...
    return await localDb.staticCrateActionReadStd({
            lonLatLoc: {
                $near: {
                    $geometry: {
                        type: "Point",
...
export async function isPlayerInProximity(lonLat: number[], kmDistance: number, playerName: string): Promise<boolean> {
    const chkPlayers = await localDb.unitActionRead({
        dead: false,
        lonLatLoc: {
            $geoWithin: {
                $centerSphere: [
