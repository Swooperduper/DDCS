    ...
    if (intCargoType === "unpack") {
        ...
        if (curUnit.inAir) {
            ...
            if (curIntCrateType) {
                ...
                const bases = await localDb.baseActionRead({});
                for (const base of bases) {
                    const curCheckAllBase = await proxZone.isPlayerInProximity(base.centerLoc, 3.4, curUnit.playername);
                ...
                if (curIntCrateBaseOrigin === curBaseName) {
                    ...
                    if (curIntCrateType === "BaseRepair") {
                        if (_.some(playerProx)) {
                            await menu.repairBase(curBaseObj, curUnit);
                        } else {
                            await playerLib.sendMesgToGroup(
                                curUnit.groupId,
                    ...
                    if (curIntCrateType === "CCBuild") {  // serverName, curUnit, curPlayer, intCargoType
                        await constants.getServer();
                        const delCrates = await localDb.staticCrateActionRead({playerOwnerId: curPlayer.ucid});
                        for (const crate of delCrates) {
                            if (crateCount > constants.config.maxCrates - 2) {
                                await localDb.staticCrateActionDelete({
                                    _id: crate._id
                                });
                                await spawn.destroyUnit(crate._id);
                            }
                            crateCount++;
                        }
                        crateObj = {
                        ...
                        await spawn.spawnLogiCrate(crateObj, true);
                        await localDb.unitActionUpdateByUnitId({unitId: curUnit.unitId, intCargoType: ""});
                        await playerLib.sendMesgToGroup(
                            curUnit.groupId,
    ...
    if (intCargoType === "loadJTAC" || intCargoType === "loadBaseRepair" || intCargoType === "loadCCBuild") {
        if (curUnit.inAir) {
            ...
            if (_.some(playerProx)) {
                ...
                const aliveLogistics = await localDb.unitActionRead({name: curBaseName + " Logistics", dead: false});
                if (aliveLogistics.length > 0 || _.includes(curBaseName, "Carrier")) {
                    if (intCargoType === "loadJTAC") {
                        await localDb.unitActionUpdateByUnitId({
...
export async function loadTroops(unitId: string, troopType: string) {
    const units = await localDb.unitActionRead({unitId});
    ...
    if (curUnit.inAir) {
        ...
        const playerProx: any [] = [];
        const bases = await localDb.baseActionRead({baseType: "MOB", side: curUnit.coalition});
        for (const base of bases) {
            const curCheckAllBase = await proxZone.isPlayerInProximity(base.centerLoc, 3.4, curUnit.playername);
            playerProx.push(curCheckAllBase);
        }
        if (_.some(playerProx)) {
            await localDb.unitActionUpdateByUnitId({unitId, troopType});
            await playerLib.sendMesgToGroup(
                curUnit.groupId,
            ...
            const secondBases = await localDb.baseActionRead({});
            const checkAllSecondBase: any[] = [];
            let curLogistic: any;
            const aliveBases = await localDb.unitActionRead({
                _id:  /Logistics/,
                dead: false,
                coalition: curUnit.coalition
            });
            for (const base of secondBases) {
                ...
                if (!!curLogistic) {
                    checkAllSecondBase.push( await proxZone.isPlayerInProximity(
...
export async function menuCmdProcess(sessionName: string, pObj: any) {
    ...
    const units = await localDb.unitActionRead({unitId: pObj.unitId});
    const curUnit = _.get(units, 0);
    if (curUnit) {
        const player = await localDb.srvPlayerActionsRead({name: curUnit.playername});
        ...
        if (curPlayer) {
            ...
            switch (pObj.cmd) {
                case "serverTimeLeft":
                    await action.timeLeft(curUnit);
                    await action.setSideLockFlags();
                    break;
                case "lookupAircraftCosts":
                    await action.lookupAircraftCosts(curPlayer.ucid);
                    break;
                case "lookupLifeResource":
                    await action.lookupLifeResource(curPlayer.ucid);
                    break;
                case "resourcePoints":
                    await action.checkResourcePoints(curPlayer);
                    break;
                case "unloadExtractTroops":
                    if (curUnit.inAir) {
                        ...
                        if (exports.isTroopOnboard(curUnit)) {
                            ...
                            const bases = await localDb.baseActionRead({baseType: "MOB", side: curUnit.coalition});
                            for (const base of bases) {
                                playerProx.push(await proxZone.isPlayerInProximity(
                                    base.centerLoc,
                                    3.4,
                                    curUnit.playername
                                ));
                            ...
                            if (_.some(playerProx)) {
                                ...
                                const curTroops: any[] = [];
                                const delUnits = await localDb.unitActionRead({
                                    playerOwnerId: curPlayer.ucid,
                                    isTroop: true,
                                    dead: false
                                });
                                for (const unit of delUnits) {
                                    ...
                                    await spawn.destroyUnit(unit.name);
                                }

                                curSpawnUnit = _.cloneDeep(await spawn.getRndFromSpawnCat(
                                    curUnit.troopType,
                                    curUnit.coalition,
                                    false,
                                    true
                                ...
                                ;
                                await spawn.spawnLogiGroup(curTroops, curUnit.coalition);
                                await playerLib.sendMesgToGroup(
                                    curUnit.groupId,
                            ...
                            const troopUnits = await proxZone.getTroopsInProximity(curUnit.lonLatLoc, 0.2, curUnit.coalition);
                            const curTroop = troopUnits[0];
                            if (curTroop) {
                                // pickup troop
                                const grpUnits = await localDb.unitActionRead({
                                    groupName: curTroop.groupName,
                                    isCrate: false,
                                    dead: false
                                });
                                for (const curTroopUnit of grpUnits) {
                                    await spawn.destroyUnit(curTroopUnit.name);
                                }
                                await localDb.unitActionUpdateByUnitId({
                                    unitId: pObj.unitId,
                ...
                case "isCrateOnboard":
                    ...
                    break;
                case "unpackCrate":
                    const logiProx = await proxZone.getLogiTowersProximity(curUnit.lonLatLoc, 0.8, curUnit.coalition);
                    if (logiProx.length) {
                        await playerLib.sendMesgToGroup(
                            curUnit.groupId,
                        ...
                        if (curUnit.inAir) {
                            ...
                            if (curUnit.inAir) {
                                ...
                                const chkPlayer = await localDb.srvPlayerActionsRead({name: curUnit.playername});
                                const curChkPlayer = chkPlayer[0];
                                if (curChkPlayer) {

                                    const crateUpdate = await localDb.staticCrateActionRead({});
                                    const sendClient = {
                                        action: "CRATEUPDATE",
                ...
                case "Mortar Team":
                    ...
                    break;
                case "acquisitionCnt":
                    const unitsOwned = await localDb.unitActionRead({
                        playerOwnerId: curPlayer.ucid,
                        isCrate: false,
                        isTroop: false,
                        dead: false
                    });
                    const serverInfo = await constants.getServer();
...
export async function spawnAtkHeli(curUnit: typing.IUnit, curPlayer: typing.ISrvPlayers, heliType: string, rsCost: number) {
    ...
    const spentPoints = await action.spendResourcePoints(curPlayer, rsCost, "AtkHeli", heliObj);
    if (spentPoints) {
        await spawn.spawnAtkChopper(curUnit, heliObj);
    }
}
...
export async function spawnAWACS(curUnit: typing.IUnit, curPlayer: typing.ISrvPlayers, awacsType: string, rsCost: number) {
    ...
    const spentPoints = await action.spendResourcePoints(curPlayer, rsCost, "AWACS", awacsObj);
    if (spentPoints) {
        await spawn.spawnAWACSPlane(curUnit, awacsObj);
    }
}
...
export async function spawnBomber(curUnit: any, curPlayer: any, bomberType: string, rsCost: number) {
    ...
    const spentPoints = await action.spendResourcePoints(curPlayer, rsCost, "Bomber", bomberObj);
    if (spentPoints) {
        await spawn.spawnBomberPlane(curUnit, bomberObj);
    }
}
...
export async function spawnCrateFromLogi(
    ...
    if (unit.inAir) {
        ...
        const serverInfo = await constants.getServer();
        const player = await localDb.srvPlayerActionsRead({name: unit.playername});
        const curPlayer = player[0];
        let closeLogi = "";
        const normalCrateBases = await localDb.baseActionRead({});
        const checkAllBase: any[] = [];
        let curLogistic;
        const aliveBases = await localDb.unitActionRead({_id:  /Logistics/, dead: false, coalition: unit.coalition});
        for (const base of normalCrateBases) {
            curLogistic = _.find(aliveBases, {name: base.name + " Logistics"});
            closeLogi = _.get(base, "name");
            if (!!curLogistic) {
                checkAllBase.push(await proxZone.isPlayerInProximity(
                    curLogistic.lonLatLoc,
                    0.2,
                    unit.playername
                ));
        ...
        if (_.some(checkAllBase)) {
            ...
            const curCrate = _.find(constants.staticDictionary, {_id: crateType});
            if (curCrate) {
...
export async function spawnDefHeli(curUnit: any, curPlayer: any, heliType: string, rsCost: number) {
    ...
    const spentPoints = await action.spendResourcePoints(curPlayer, rsCost, "DefHeli", heliObj);
    if (spentPoints) {
        await spawn.spawnDefenseChopper(curUnit, heliObj);
    }
}
...
export async function spawnTanker(curUnit: any, curPlayer: any, tankerType: string, rsCost: number) {
    ...
    remoteLoc = await proxZone.getLonLatFromDistanceDirection(curUnit.lonLatLoc, curUnit.hdg, tankerObj.spawnDistance);

    const closeMOBs1 = await proxZone.getMOBsInProximity(
        remoteLoc,
        safeSpawnDistance,
        constants.enemyCountry[curUnit.coalition]
    );
    const closeMOBs2 = await proxZone.getMOBsInProximity(
        curUnit.lonLatLoc,
        safeSpawnDistance,
        constants.enemyCountry[curUnit.coalition]
    );
    ...
    if (closeMOBs1.length > 0 || closeMOBs2.length > 0) {
        ...
        const spentPoints = await action.spendResourcePoints(curPlayer, rsCost, "Tanker", tankerObj);
        if (spentPoints) {
            await spawn.spawnTankerPlane(curUnit, tankerObj, curUnit.lonLatLoc, remoteLoc);
        }
    }
}
...
export async function unpackCrate(playerUnit: any, country: string, type: string, special: string, combo: boolean, mobile: boolean) {
    ...
    if (playerUnit.inAir) {
        ...
        const player = await localDb.srvPlayerActionsRead({name: playerUnit.playername});
        const curPlayer = player[0];
        const delUnits = await localDb.unitActionReadStd({
            playerOwnerId: curPlayer.ucid,
            playerCanDrive: mobile,
            isCrate: false,
            dead: false
        ...
        for (const gUnitKey of Object.keys(grpGroups)) {
            if (curUnit <= tRem) {
                for (const unit of grpGroups[gUnitKey]) {
                    await localDb.unitActionUpdateByUnitId({unitId: unit.unitId, dead: true});
                    await spawn.destroyUnit(unit.name);
                }
                curUnit++;
            }
        ...
        if (combo) {
            const unitDic = await constants.getUnitDictionary(curTimePeriod);
            const addHdg = 30;
            ...
            await spawn.spawnLogiGroup(newSpawnArray, playerUnit.coalition);
            return true;
        } else {
            const unitDic = await constants.getUnitDictionary(curTimePeriod);
            const addHdg = 30;
